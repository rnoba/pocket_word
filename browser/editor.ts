...
const editor_camera: Base.Camera = {
	width:	0,	
	height: 0, 
	x: 0, y: 0, z: 0,
...
export function editor_set_size(w: number, h: number)
{
...
function editor_update_cursor()
{
	...
	Base.camera_transform_world(editor_camera,
															cursor.position.x,
															cursor.position.y,
															0,
...
function get_selection(out: Base.Ptr<Base.Rect>): void
{
	const drag_start	= Ui.ui_state.drag_start;
	const delta				= Ui.ui_drag_delta();
	const start_world = Base.camera_transform_world(editor_camera,
																									drag_start.x,
...
function editor_get_mode()
{
	...
	if (Input.is_down(Input.KKey.KEY_Shift))	{ mode = editor_state.selections.length > 0 ? EditorMode.EditMode : EditorMode.CreationMode; }
	if (Input.is_down(Input.KKey.KEY_Ctrl))	{ mode = EditorMode.SelectionMode; }
	return (mode);
}
...
function editor_handle_free_mode()
{
	...
	if (scroll_y !== 0)
	{
		editor_camera.zoom = Base.Clamp(
			editor_camera.zoom - (0.5 * Math.sign(scroll_y)),
			0.3,
			2
		);
	}
...
const r: Base.Ptr<Base.Rect> = Base.Ptr(Base.Rect(0, 0, 0, 0)); 
...
function editor_handle_selection_mode()
{
	for (let i = editor_state.shapes.length - 1; i >= 0; i -= 1)
	{
		...
		for (let j = shape.tiles.length - 1; j >= 0; j -= 1)
		{
			...
			const selection: Selection = { kind: SelectionKind.None, data: null };
			...
			if (hovering_top_face && can_select)
			{
				...
				selection.data = { face: Faces.Top, value: tile.faces.top, parent_id: shape.id, tile } as FaceSelection;
			}
			else if (hovering_left_face && can_select)
			{
				selection.kind = SelectionKind.Face;
				selection.data = { face: Faces.Left, value: tile.faces.left, parent_id: shape.id, tile } as FaceSelection;
			}
			else if (hovering_right_face && can_select)
			{
				selection.kind = SelectionKind.Face;
				selection.data = { face: Faces.Right, value: tile.faces.right, parent_id: shape.id, tile } as FaceSelection;
			}
	...
	if (Input.is_pressed(Input.KKey.KEY_V))
	{
		if (editor_state.selections.length > 0)
		{
			for (let i = 0; i < editor_state.selections.length; i += 1)
			{
				...
				{
					const shape = editor_state.selections[i].data as Shape;
...
function shape_add_walls(shape: Shape)
{
...
const edit_mode_selection: Base.Ptr<Base.Rect> = Base.Ptr(Base.Rect(0, 0, 0, 0)); 
...
function draw_shapes()
{
	for (let i = 0; i < editor_state.shapes.length; i += 1)
	{
		...
		for (let j = 0; j < shape.tiles.length; j += 1)
		{
			...
			if ((shape.fill_sprite && tile.sprite === null) || shape.refresh_fill) { tile.sprite = shape.fill_sprite; }
			tile_draw(tile,
								Ui.ui_state.bitmap_stack.top!.value!,
								shape.offset.x,
								shape.offset.y,
								shape.layer); 
		}
...
export function editor(dt: number, test_bitmap: Sprite.SpriteSource)
{
...
type EditorState = {
	...
	mode: EditorMode;
	root_interaction_dragging: boolean;
	root_interaction_clicked: boolean;
	root_interaction_dclicked: boolean;
	scroll_y: number;
	dt: number;
}

const WALL_CORNER = Sprite.Sprite_new(
			Base.Rect(128, 64, 64, 64),
			"test6.png");

const TEST_WALLS = [
	[
		Sprite.Sprite_new(
			Base.Rect(0, 64, 64, 64),
			"test6.png",
		),
		Sprite.Sprite_new(
			Base.Rect(64, 64, 64, 64),
			"test6.png",
		),
	]
];
...
const TEST_STAIRS = [
	[
		Sprite.Sprite_new(
			Base.Rect(0, 128, 64, 64),
			"test6.png"),
		Sprite.Sprite_new(
			Base.Rect(64, 128, 64, 64),
			"test6.png"),
	]
]

const TEST_FLOORS: Sprite.Sprite[] = [
	Sprite.Sprite_new(
		Base.Rect(0, 0, 64, 64),
		"test6.png",
	),
...
function editor_remove_shape(shape_id: string)
{
	for (let i = 0; i < editor_state.shapes.length; i += 1)
	{
...
function find_shape_by_id(id: string)
{
...
function foreach_selection(cb: (sel: Selection) => void)
{
	for (let i = 0; i < editor_state.selections.length; i += 1)
	{
...
function remove_selection(sel: Selection)
{
	let i = 0;
	for (i; i < editor_state.selections.length; i += 1)
	{
		...
		{
			const data = selection.data as FaceSelection;
			const new_ = sel.data				as FaceSelection;
...
function already_selected(sel: Selection): boolean
{
	let ret = false;
	for (let i = 0; i < editor_state.selections.length; i += 1)
	{
...
function is_shape_selected(shape: Shape)
{
	let ret = false;
	for (let i = 0; i < editor_state.selections.length; i += 1)
	{
...
function is_face_selected(face: Base.ConvexQuadrilateral)
{
	let ret = false;
	for (let i = 0; i < editor_state.selections.length; i += 1)
	{
...
function is_tile_selected(tile: Tile)
{
	let ret = false;
	for (let i = 0; i < editor_state.selections.length; i += 1)
	{
									 ...
									 layer_offset: number = 0)
{
	if (tile.sprite && src)
	{
		const screen_position =  Base.camera_transform_screen(editor_camera,
