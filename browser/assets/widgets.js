...
export function default_container_begin(text, flags, width = Ui.size_grow(), height = Ui.size_grow(), palette = Palette.vaporwave) {
...
const sizes = new Map();
export function resizable_begin(text, axis) {
...
export function editor_draw(parent) {
    ...
    Ui.push_next_size(Ui.AxisY, Ui.size_pct(1));
    let axis = Ui.AxisX;
    if (parent.fixed_size[Ui.AxisX] < 1000) {
        axis = Ui.AxisY;
    }
...
export function editor(dt, state, root) {
    if (root) {
        Ui.push_parent(root);
        editor_draw(root);
        Ui.pop_parent();
