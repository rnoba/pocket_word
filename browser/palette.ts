...
export interface Palette {
	...
	hot_background_color: Base.RGBA;
	active_background_color: Base.RGBA;

	hot_border_color: Base.RGBA;
	active_border_color: Base.RGBA;

	hot_border_size: number;
	active_border_size: number;

	hot_rouded_corners: Ui.RoundedCornersRadius;
	active_rouded_corners: Ui.RoundedCornersRadius;

	hot_font: string;
	active_font: string;

	hot_font_size: number;
	active_font_size: number;

	hot_text_color: Base.RGBA;
	active_text_color: Base.RGBA;
}
...
export function custom(palette: Palette, settings: Partial<Palette>, purge: boolean = false): Palette {
	const n: Palette = { ...palette, ...settings };

	if (purge)
	{
		for (const k of Object.keys(settings)) {
			const hot_k =`hot_${k}` as keyof Palette; 
			const active_k =`active_${k}` as keyof Palette; 

			// @ts-ignore
			if (hot_k in palette) { n[hot_k] = settings[k as keyof Palette]!; }
			// @ts-ignore
			if (active_k in palette) { n[active_k] = settings[k as keyof Palette]!; }
		}
	}
	return n as Palette;
}
...
export const ocean = palette(
	Base.Hex("#0a2e40"),  // Deep navy background
	Base.Hex("#4db8ff"),  // Vibrant sky blue border
	4,
	Ui.rouded_corners(5, 5, 5, 5),
	"Roboto",
	16,
	Base.Hex("#ffffff")
);
...
export const sunset = palette(
	Base.Hex("#ffcc66"),  // Warm, glowing background
	Base.Hex("#ff6666"),  // Soft coral border
	3,
	Ui.rouded_corners(6, 6, 6, 6),
	"Montserrat",
	18,
	Base.Hex("#333333")
);
...
export const forest = palette(
	Base.Hex("#2e8b57"),  // Lush green background
	Base.Hex("#556b2f"),  // Earthy, dark olive border
	4,
	Ui.rouded_corners(7, 7, 7, 7),
	"Georgia",
	17,
	Base.Hex("#f0f0f0")
);
...
export const neon = palette(
	Base.Hex("#1a1a1a"),  // Dark background to let neon pop
	Base.Hex("#39ff14"),  // Neon green border for a vibrant look
	2,
	Ui.rouded_corners(10, 10, 10, 10),
	"Courier New",
	14,
	Base.Hex("#e0e0e0")
);
...
export const midnight_aura = palette(
	Base.Hex("#1A1B26"),   // Cosmic navy background
	Base.Hex("#2AC3DE"),   // Electric teal border
	3,
	Ui.rouded_corners(8, 8, 8, 8),
	"Inter",
	16,
	Base.Hex("#FFFFFF")
);
...
export const minimalist = palette(
	Base.Hex("#F5F5F5"),   // Crisp white background
...
export const desert = palette(
	Base.Hex("#EDCBB1"),   // Sunbaked clay background
	Base.Hex("#C44536"),   // Terracotta border
	4,
	Ui.rouded_corners(12, 12, 12, 12),
	"Lora",
	17,
	Base.Hex("#4A2B21")
);
...
export const cyberpunk = palette(
	Base.Hex("#0A0F2C"),   // Deep space blue background
	Base.Hex("#FF007F"),   // Neon pink border
	4,
	Ui.rouded_corners(10, 10, 10, 10), // Angular cyberpunk style
	"Fira Code",
	14,
	Base.Hex("#E0E0E0")
);
...
export const pastel = palette(
	Base.Hex("#D1F0E8"),   // Mint cream background
	Base.Hex("#B19CD9"),   // Lavender border
	3,
	Ui.rouded_corners(16, 16, 16, 16),
	"Quicksand",
	16,
	Base.Hex("#4A4A4A")
);
...
export const accessible = palette(
	Base.Hex("#FFD700"),   // Vivid yellow background
...
export const vaporwave = palette(
	Base.Hex("#FF9A8B"),   // Peach glow background
	Base.Hex("#00CED1"),   // Retro teal border
	5,
	Ui.rouded_corners(20, 20, 20, 20),
	"Bebas Neue",
	20,
	Base.Hex("#2A2A2A")
);
...
export const professional = palette(
	Base.Hex("#2D2D2D"),   // Deep gray background
